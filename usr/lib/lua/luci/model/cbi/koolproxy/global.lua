...
local a=luci.sys.exec("head -3 /usr/share/koolproxy/data/rules/koolproxy.txt | grep rule | awk -F' ' '{print $3}'")
local b=luci.sys.exec("grep -v !x /usr/share/koolproxy/data/rules/koolproxy.txt | wc -l")
local c=luci.sys.exec("head -5 /usr/share/koolproxy/data/rules/ad.txt | grep Version | awk -F' ' '{print $3}'")
local d=luci.sys.exec("grep -v !x /usr/share/koolproxy/data/rules/ad.txt | wc -l")
local s=luci.sys.exec("head -2 /usr/share/koolproxy/data/rules/anti-ad.txt | grep Version | awk -F' ' '{print $2}'")
local m=luci.sys.exec("grep -v !x /usr/share/koolproxy/data/rules/anti-ad.txt | wc -l")
local u=luci.sys.exec("head -4 /usr/share/koolproxy/data/rules/koolproxy.txt | grep video | awk -F' ' '{print $3}'")
local p=luci.sys.exec("grep -v !x /usr/share/koolproxy/data/rules/tvbox.txt | wc -l")
local l=luci.sys.exec("head -3 /usr/share/koolproxy/data/rules/dykpr.txt | grep rules | awk -F' ' '{print $3,$4}'")
local q=luci.sys.exec("head -4 /usr/share/koolproxy/data/rules/dykpr.txt | grep video | awk -F' ' '{print $3,$4}'")
local f=luci.sys.exec("grep -v !x /usr/share/koolproxy/data/rules/dykpr.txt | wc -l")
...
if luci.sys.call("pidof koolproxy >/dev/null") == 0 then
	status = translate("<strong class=\"koolproxy_status\"><font color=\"green\">KoolproxyM 运行中</font></strong>")
else
	status = translate("<strong class=\"koolproxy_status\"><font color=\"red\">KoolproxyM 未运行</font></strong>")
end

o = Map("koolproxy", "<font color='green'>" .. translate("KoolproxyM") .."</font>",     "<font color='purple'>" .. translate( "KoolProxyM是一款高效的基于规则过滤上网流量包的软件，用于过滤页面垃圾推广元素、不良信息、不健康内容等，并且支持https！") .."</font>")
...
e.optional = false
e.rmempty = false
e:value("koolproxy.txt", translate("koolproxy+daily规则"))
e:value("ad.txt", translate("乘风视频广告过滤、EasylistChina、EasylistLite、CJX'sAnnoyance规则"))
e:value("anti-ad.txt", translate("ANTI-AD规则"))
e:value("kp.dat", translate("KP视频规则"))
e:value("tvbox.txt", translate("电视盒子规则"))
e:value("dykpr.txt", translate("DYKPR视频规则"))
...
e = t:taboption("base", ListValue, "time_update", translate("定时更新所有订阅规则"))
...
e = t:taboption("base", Button, "0gen", translate(".0根证书"))
e.inputtitle = translate(".0根证书下载")
e.inputstyle="reload"
e.write=function()
	luci.sys.call("/usr/share/koolproxy/camanagement kp_cert_0 >/dev/null")
	Download0()
...
e.description = translate(string.format("<font color=\"red\"><strong>更新订阅规则</strong></font><br /><font color=\"green\">koolproxy+daily规则:%s %s条<br />规则合集:%s %s条<br />ANTI-AD规则:%s %s条<br />KP视频规则: %s<br />电视盒子规则: %s条<br />DYKPR视频规则: RULES:%s VIDEO:%s %s条<br />自定义规则: %s条<br />Host: %s条</font><br />", a, b, c, d, s, m, u, p, l, q, f, h, i))
...
function Download0()
	local t,e
	t=nixio.open("/tmp/upload/ca_0.tar.gz","r")
	luci.http.header('Content-Disposition','attachment; filename="ca_0.tar.gz"')
	luci.http.prepare_content("application/octet-stream")
	while true do
		e=t:read(nixio.const.buffersize)
		if(not e)or(#e==0)then
